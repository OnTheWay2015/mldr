...
typedef struct __MEMMODULE_S {
  ...
  int bCallEntry;     // Call module entry type  = 0, none; =1, dll; =2,exe; =3,exe_win32
  BOOL bLoadOk;        // MemModule is loaded ok?
  DWORD dwErrorCode;   // Last error code
} MEM_MODULE, *PMEM_MODULE;

BOOL
LoadMemModuleInternal(PMEM_MODULE pMemModule, LPVOID lpPeModuleBuffer, int bCallEntry);
...
BOOL
CallExeEntry(PMEM_MODULE pMemModul);

BOOL
CallExeWin32Entry(PMEM_MODULE pMemModul);
...
MemModuleHelper(_In_ MMHELPER_METHOD method, _In_ LPVOID lpArg1, _In_ LPVOID lpArg2, _In_ LPVOID lpArg3) {
  switch (method) {
  case MHM_BOOL_LOAD: {
        //@lpArg2 是否在加载后调用 DLLMain()
    //return (LPVOID)(INT_PTR)LoadMemModule(lpArg1, (BOOL)(lpArg2 != 0), (DWORD *)lpArg3);
    return (LPVOID)(INT_PTR)LoadMemModule(lpArg1, *((int*)lpArg2), (DWORD *)lpArg3);
  } break;
  case MHM_VOID_FREE: {
...
LoadMemModuleInternal(PMEM_MODULE pMemModule, LPVOID lpPeModuleBuffer, int bCallEntry) {
  ...
  else if (bCallEntry == 2) {
    //exe 
    if (FALSE == CallExeEntry(pMemModule)) {
      // failed to call entry point,
      // clean resource, return false
      UnmapMemModule(pMemModule);
      return FALSE;
  ...
  else if (bCallEntry == 3) {
    //exe_win32
    if (FALSE == CallExeWin32Entry(pMemModule)) {
      // failed to call entry point,
      // clean resource, return false
      UnmapMemModule(pMemModule);
      return FALSE;
...
HMEMMODULE
LoadMemModule(_In_ LPVOID lpPeModuleBuffer, _In_ int bCallEntry, _Inout_ DWORD *pdwError) {
...
BOOL
CallExeEntry(PMEM_MODULE pMemModule ) {
  if (NULL == pMemModule || NULL == pMemModule->pImageDosHeader)
    return FALSE;
  ...
  pfnModuleEntry = MakePointer(mainfun, pMemModule->lpBase, pImageNtHeader->OptionalHeader.AddressOfEntryPoint);
...
int _nCmdShow;
void
set_win32_params(HINSTANCE hInstance,
...
BOOL
CallExeWin32Entry(PMEM_MODULE pMemModule) {
  // todo
  ...
  //typedef int (*mainfun)(_In_ HINSTANCE hInstance,
  //                   _In_opt_ HINSTANCE hPrevInstance,
  //                   _In_ LPWSTR    lpCmdLine,
  //                   _In_ int       nCmdShow);
  typedef int (*mainfun)();//pe入口方法在程序入口之前,没有参数
  mainfun pfnWin32Entry = NULL;
  ...
  pfnWin32Entry = MakePointer(mainfun , pMemModule->lpBase, pImageNtHeader->OptionalHeader.AddressOfEntryPoint);

  if (NULL == pfnWin32Entry) {
    pMemModule->dwErrorCode = MMEC_INVALID_ENTRY_POINT;
    return FALSE;
  }
  return pfnWin32Entry();
}
